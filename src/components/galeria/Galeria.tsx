...
import { CarrosselGaleria } from "@/components/galeria/CarrosselGaleria";
...
/** Galeria de fotos e vídeos: vitrine por categoria, ou grade ao filtrar/buscar. */
export function Galeria() {
  const [categoria, setCategoria] = useState<ItemGaleria["categoria"] | "">("");
  const semFiltro = !categoria;
  ...
  return (
    <div className="section-y">
      ...
      {semFiltro ? (
        <div className="mt-8">
          {CATEGORIAS_GALERIA.map((cat) => (
            <CarrosselGaleria
              key={cat}
              titulo={cat}
              itens={GALERIA.filter((item) => item.categoria === cat).sort(
                (a, b) => new Date(b.data).getTime() - new Date(a.data).getTime(),
              )}
            />
          ))}
        ...
        <div className="container-portal mt-8">
